...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public Model.WorkInfoNo GetModel(int id)
        {
            ...
            {
                string sql = "select workinfo.*,activity.activityName from workinfo join activity on workinfo.activityId=activityId where workId=@id";
        ...
        public int Update(Model.WorkInfo workInfo)
        {
            using (IDbConnection cn =new MySqlConnection(cns))
            {
                string sql = "insert into workinfo(workId,workName,workPicture,uploadTime=@uploadTime,workIntroduction=@workIntroduction,workVerify=@workVerify," + "userName=@userName,recommend=@recommend,recommendTime=@recommendTime,workId=@workId";
        ...
        public int UpdateImg(Model.WorkInfo workInfo)
        {
            using (IDbConnection cn=new MySqlConnection(cns))
            {
                ...
                if (workInfo.workPicture == null)
                    sql = "update workinfo ser workIntroduction=@workIntroduction where workId=@workId";
                return cn.Execute(sql, workInfo);
            }
        }
